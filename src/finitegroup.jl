...
"""
    expand(G::FiniteGroup{E}, g::E) -> FiniteGroup{E}

Expand a finite group by adding a new element and generating the group closure.

Returns a new finite group that contains `G` as a subgroup and is obtained by 
adding the element `g` to the generating set of `G`, then computing all elements 
that can be generated by group composition.

# Arguments
- `G::FiniteGroup{E}`: The finite group to expand
- `g::E`: The group element to add to the generating set

# Returns
- `FiniteGroup{E}`: A new finite group containing all elements of `G` and all 
  elements generated by composing `g` with elements of `G`

# Note
If `g` is already in `G`, the returned group will be identical to `G`. Otherwise,
the returned group will be strictly larger than `G`.
"""    
function expand(G::FiniteGroup{E}, g::E)::FiniteGroup{E} where E<:GroupElement
    e=copy(G.e)
    push!(e, g)
    FiniteGroup{E}(e)
end
